...
router.get('/:username', async (req, res) => {
    ...
    const userDetailsResponse = await fetch(`${process.env.GITHUB_USER_BASE_URL}/${username}`, {
      headers: {
          Authorization: `token ${process.env.GITHUB_TOKEN}`,
      },
  });
  const userReposResponse = await fetch(`${process.env.GITHUB_USER_BASE_URL}/${username}/repos`, {
    headers: {
        Authorization: `token ${process.env.GITHUB_TOKEN}`,
    },
});
